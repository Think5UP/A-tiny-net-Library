...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
    ...
    if (!benchmark){
        const std::map<std::string, std::string>& headers = req.Header();
    ...
    if (req.getPath() == "/")  // 根目录请求
    {
        ...
        std::string now = net_lib::Timestamp::now().toFormattedString();
        resp->setBody("<html><head><title>This is title</title></head>"
                      "<body><h1>Hello</h1>Now is " + now +
                      "</body></html>");
    }
    else if (req.getPath() == "/favicon.ico")  // 图标请求
    {
        resp->setStatusCode(HttpResponse::k2000k);
        resp->setStatusMessage("OK");
...
int main(int argc, char* argv[])
{
    ...
    net_lib::EventLoop loop;
    HttpServer server(&loop, net_lib::InetaAddress(8080,"0.0.0.0"), "dummy");
